...
plot(dados_HW, ylab = "Consumo de Energia Elétrica (MWh)", xlab = "Ano")
...
plot(fitted(dados_HW), ylab = "Consumo de Energia Elétrica (MWh)", xlab = "Ano")
...
forecast:::plot.forecast(dados_forecast, ylab = "Consumo de Energia Elétrica (MWh)", xlab = "Ano")
...
#leitura da base de dados: (valores em arquivo txt)
dataset_energia_eletrica_MATRIX <- read_excel("~/Facul/Iniciação Científica/Códigos R/dataset_energia_eletrica_MATRIX.xlsx")
...
#cria o objeto elm - aplica o método lasso para seleção de variáveis
dados_elm <- elm(dados_ts, m = frequency(dados_ts), hd = NULL, type = c("lasso"), reps = 20, comb = "mean", lags = NULL)

plot(dados_elm)

#realiza a previsão da série temporal com base no modelo ELM gerado
forecast_elm <- forecast(dados_elm, h = 12)

#plot da previsão calculada
