...
plot.ts(dados_ts, ylab = "Produção de gás natural (10^3 m3)", xlab = "Ano")
#aplicação do método de Holt-Winters aditivo
dados_HW <- HoltWinters(dados_ts, seasonal = c("additive"))
plot(dados_HW, ylab = "Produção de gás natural (10^3 m3)", xlab = "Ano")
...
plot(fitted(dados_HW),ylab = "Produção de gás natural (10^3 m3)", xlab = "Ano")
...
forecast:::plot.forecast(dados_forecast,ylab = "Produção de gás natural (10^3 m3)", xlab = "Ano")
...
#SE DER ERRO USAR A OPÇÃO do RStudio: 'Import Dataset" -> 'From Excel" -> escolher arquivo
dataset_gas_natural_MATRIX <- read_excel("~/Facul/Iniciação Científica/Códigos R/dataset_gas_natural_MATRIX.xlsx")
#valores de 2019 são descartados, pois o método falha quando há espaços da matriz marcados como NA
...
dados <- read_excel("~/Facul/Iniciação Científica/Códigos R/dataset_gas_natural_MATRIX2.xlsx")
...
if (problem == 2) {   #série temporal
  ...
  atraso       <- 2   # atraso no sinal de entrada do modelo
